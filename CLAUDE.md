# 隔日 System Design 教练 — 项目说明

每隔一天生成一期深度 system design 学习内容，以精美 HTML 发布到 GitHub。

## 核心定位
**工程师友好的 system design 学习**。每期一个主题，包含 3-4 个关键点：原理、选型 trade-off、现实案例拆解、面试题示范。
...
4. **包含能画出来的架构**。用 ASCII 或清晰的文字描述数据流、组件关系。
5. **能写出伪代码**。Python/Go 伪代码说明关键代码路径。
6. **指出面试讨论点**。这个主题在面试中会怎么问、主面试官期望什么、常见陷阱。
7. 长度：整页 1500-2200 字。

## 主题顺序（从基础到高阶）
检查仓库已有的 `*-day*.html` 文件，找第一个缺失的编号。按以下顺序选主题：
...
## HTML 输出格式
生成精美 HTML。使用内联 CSS，风格现代技术感（背景 `#0e1419` 深色、主色 `#64c8ff` 青蓝 / `#5eead4` 青绿、代码块使用 SF Mono 等宽字体、架构图用 ASCII / box-drawing 字符）。移动端响应式。

### 页面结构
- **顶部**：主题（中英文）+ Day 编号 + 难度（Easy/Medium/Hard）+ 领域 tag
- **【问题场景】**：什么产品需要这个，举一两个现实案例
- **【需求与约束】**：面试中要问的清化问题、QPS、数据量、延迟要求
- **【高层架构】**：主要组件与数据流，用 ASCII / box-drawing 画架构图
- **【关键技术点】**：3-4 个技术点，每个包含：
  - **【原理】**：什么原理，为什么这样设计
  - **【Trade-off】**：与替代方案的权衡
  - **【代码/伪代码】**：Python/Go 示例
...
## 文件命名
`主题英文-dayX.html`，例如 `caching-day2.html`, `sharding-day4.html`
